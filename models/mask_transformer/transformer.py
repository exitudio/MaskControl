...
class MaskTransformer(nn.Module):
    ...
    def forward(self, y, m_lens, pose, vq_model):
        '''
        :param ids: (b, n)
        ...
        :return:
        '''
        x_encoder = vq_model.encoder(pose.permute(0, 2, 1))
        code_idx = vq_model.quantizer.quantize(x_encoder)
        ids = code_idx[..., 0]
        ...
        labels = torch.where(non_pad_mask, ids, self.mask_id) # xent to all tokens (not just masked position)
        ...
        std = .1
        num_cb = vq_model.quantizer.codebooks[0].shape[0]
        ...
        logits = self.trans_forward(trans_emb, cond_vector, ~non_pad_mask, force_mask)
        ce_loss, pred_id, acc = cal_performance(logits, labels, ignore_index=self.mask_id)

        ###### Embedding loss ######
        padding_mask = ~non_pad_mask
        emb = F.softmax(logits.permute(0,2,1)/1, dim=-1) @ vq_model.quantizer.codebooks[0]
        emb = emb.masked_fill(padding_mask.unsqueeze(-1), 0.)
        loss_emb = F.l1_loss(x_encoder.permute(0,2,1)[non_pad_mask], emb[non_pad_mask], reduction='mean')

        return loss_emb, ce_loss, pred_id, acc
    ...
    def forward_with_cond_scale(self,
        ...
        # if cond_scale == 1:
        if force_mask:
            return self.trans_forward(motion_ids, cond_vector, padding_mask, force_mask=True, ctrlNet_cond=ctrlNet_cond)

        logits = self.trans_forward(motion_ids, cond_vector, padding_mask, ctrlNet_cond=ctrlNet_cond)
        if cond_scale == 1:
            return logits

        aux_logits = self.trans_forward(motion_ids, cond_vector, padding_mask, force_mask=True, ctrlNet_cond=ctrlNet_cond)
    ...
    def generate(self,
        ...
        filtered_logits = torch.ones(emb.shape, device=emb.device, dtype=torch.float)
        ...
        for timestep, steps in zip(torch.linspace(0, 1, timesteps, device=device), range(timesteps)):
            ...
            if gsample:  # use gumbel_softmax sampling
                ...
                pred_emb = probs @ self.token_emb.weight[:emb.shape[-1]]
...
class ResidualTransformer(nn.Module):
    ...
    def trans_forward(self, motion_codes, qids, cond, padding_mask, force_mask=False, ctrlNet_cond=None):
        '''
    ...
    def forward_with_cond_scale(self,
        ...
        if force_mask:
            logits = self.trans_forward(motion_codes, qids, cond_vector, padding_mask, force_mask=True, ctrlNet_cond=ctrlNet_cond)
            logits = self.output_project(logits, qids-1)
            return logits

        logits = self.trans_forward(motion_codes, qids, cond_vector, padding_mask, ctrlNet_cond=ctrlNet_cond)
        logits = self.output_project(logits, qids-1)
        if cond_scale == 1:
            return logits

        aux_logits = self.trans_forward(motion_codes, qids, cond_vector, padding_mask, force_mask=True, ctrlNet_cond=ctrlNet_cond)
    ...
    def generate(self,
        ...
        for i in range(1, num_quant_layers):
            ...
            if i == 0:
                ...
                temp = 1e-8
            _emb = F.softmax(logits/temp, dim=-1) @ token_embed
